...
public class GradingSetting : BaseEntity
{
    ...
    public string SchoolLevelName { get; private set; } = string.Empty;
    ...
    public GradingSetting(int subjectId, string schoolLevelName, decimal maxCAScore, decimal maxExamScore, decimal minimumA1, decimal minimumB2, decimal minimumB3, decimal minimumC4, decimal minimumC5, decimal minimumC6, decimal minimumD7, decimal minimumE8)
    {
        if (subjectId <= 0)
            throw new ArgumentException("Subject ID is invalid.");
        SubjectId = subjectId;
        UpdateGradingSetting(schoolLevelName, maxCAScore, maxExamScore, minimumA1, minimumB2, minimumB3, minimumC4, minimumC5, minimumC6, minimumD7, minimumE8);
    }

    public void UpdateGradingSetting(string schoolLevelName, decimal maxCAScore, decimal maxExamScore, decimal minimumA1, decimal minimumB2, decimal minimumB3, decimal minimumC4, decimal minimumC5, decimal minimumC6, decimal minimumD7, decimal minimumE8)
    {
        // 1. Structural score allocation check
        if (maxCAScore + maxExamScore != 100)
            throw new ArgumentException("Total maximum score must equal exactly maximum total score.");
        ...
        if(string.IsNullOrWhiteSpace(schoolLevelName))
            throw new ArgumentException("School level name cannot be empty.");

        SchoolLevelName = schoolLevelName.Trim();
